...
class OrdersService(SupabaseService):
    ...
    def _validate_order_business_rules(self, order_data: order_schema.OrderCreate) -> None:
        ...
        if abs(order_data.value_ordered - expected_value) > 0.01:  # Tolérance pour les arrondis
            raise ValueError("La valeur commandée ne correspond pas à quantité × prix unitaire")
    ...
    def _validate_filter_values(self, status: str | None) -> str | None:
        ...
        if status:
            if status in ['pending', 'confirmed', 'completed', 'cancelled']:
                ...
                raise ValueError(f"Status invalide: {status}. Valeurs autorisées: pending, confirmed, completed, cancelled")
    ...
    def create_order(self, order_data: order_schema.OrderCreate) -> dict[str, Any]:
        ...
        try:
            ...
            order_response = self.client.table(self.orders_table).insert(order_dict).execute()
    ...
    def get_order_by_id(self, order_id: int) -> dict[str, Any]:
        ...
        try:
            response = self.client.table(self.orders_table).select("*, ingredients(*)").eq("id", order_id).eq("delete", False).execute()
    ...
    def update_order(self, order_id: int, update_data: order_schema.OrderUpdate) -> dict[str, Any]:
        ...
        try:
            ...
            if update_dict:
                response = self.client.table(self.orders_table).update(update_dict).eq("id", order_id).execute()
    ...
    def soft_delete_order(self, order_id: int) -> dict[str, str]:
        ...
        try:
            ...
            # Suppression logique
            _ = self.client.table(self.orders_table).update({"delete": True}).eq("id", order_id).execute()
    ...
    def adjust_stock(self, order_id: int, adjustments: order_schema.StockAdjustmentRequest) -> dict[str, Any]:
        ...
        try:
            ...
            for adjustment in adjustments.adjustments:
                ...
                if adjustment.order_id != order_id:
                    raise ValueError(f"L'ajustement ne concerne pas la commande {order_id}")
                
                # Calcul de la nouvelle valeur reçue
                new_value_received = adjustment.new_quantity_received * adjustment.new_unit_price_received
                ...
                if adjustment.reason:
                    ...
                    if isinstance(current_notes, str):
                        update_data["notes"] = f"{current_notes}\nAjustement: {adjustment.reason}" if current_notes else f"Ajustement: {adjustment.reason}"
                    else:
                        update_data["notes"] = f"Ajustement: {adjustment.reason}"
                
                _ = self.client.table(self.orders_table).update(update_data).eq("id", order_id).execute()
    ...
    def get_ingredients(self) -> list[dict[str, Any]]:
        ...
        try:
            response = self.client.table(self.ingredients_table).select("*").eq("delete", False).execute()
